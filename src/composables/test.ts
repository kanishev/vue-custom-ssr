...
> {
    ...
    const handler =
        ...
        !nuxtApp.ssrContext?._sharedPrerenderCache
            ...
            : () => {
                  const value =
                      nuxtApp.ssrContext!._sharedPrerenderCache!.get(key);
                  if (value) {
                      return value as Promise<ResT>;
                  }

                  const promise = Promise.resolve().then(() =>
                      nuxtApp.runWithContext(_handler)
                  );
    ...
    const asyncData = { ...nuxtApp._asyncData[key] } as {
        _default?: unknown;
    } & AsyncData<
        DataT | DefaultT,
        NuxtErrorDataT extends Error | NuxtError
    ...
    asyncData.refresh = asyncData.execute = (opts = {}) => {
        ...
        if (opts._initial || (nuxtApp.isHydrating && opts._initial !== false)) {
            const cachedData = opts._initial
                ? initialCachedData
            ...
            .then(async (_result) => {
                ...
                if (
                    ...
                    console.warn(
                        ...
                        }${explanation}\` must return a value (it should not be \`undefined\`) or the request may be duplicated on the client side.`
                    );
                }
    ...
    if (import.meta.client) {
        ...
        if (
            import.meta.dev &&
            !nuxtApp.isHydrating &&
            !nuxtApp._processingMiddleware /* internal flag */ &&
        ...
        if (
            fetchOnServer &&
            nuxtApp.isHydrating &&
            (asyncData.error.value || typeof initialCachedData !== "undefined")
        ) {
            // 1. Hydration (server: true): no fetch
            asyncData.pending.value = false;
            asyncData.status.value = asyncData.error.value
                ? "error"
                : "success";
        } else if (
            instance &&
    ...
    const asyncDataPromise = Promise.resolve(
        nuxtApp._asyncDataPromises[key]
    ).then(() => asyncData) as AsyncData<
        ResT,
        NuxtErrorDataT extends Error | NuxtError
            ? NuxtErrorDataT
            : NuxtError<NuxtErrorDataT>
...
function clearNuxtDataByKey(nuxtApp: NuxtApp, key: string): void {
    ...
    if (nuxtApp._asyncData[key]) {
        nuxtApp._asyncData[key]!.data.value = unref(
            nuxtApp._asyncData[key]!._default()
        );
